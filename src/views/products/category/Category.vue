<template>
  <header class="bg-white shadow">
    <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
      <h1 class="text-3xl font-bold tracking-tight text-gray-900">Categoria Produto</h1>
    </div>
  </header>
  <main>
    <div class="mx-auto max-w-7xl py-6 sm:px-6 lg:px-8 grid grid-cols-2 gap-4">
      ...
      <div>
        <Table>
          ...
          <TableBody>
            <TableRow v-for="row in category" :key="row.id_categoria">
              <TableCell class="font-medium">{{ row.id_categoria }}</TableCell>
              <TableCell>{{ row.nome }}</TableCell>
              <TableCell>
                <Button variant="destructive" @click="deleteCategory(row.id_categoria)" >
...
const addCategory = async () => {
  try {
    await api.post('/produto/categorias', {
      nome: name.value
    })
    name.value = '' 
...
const deleteCategory = async (categoryId: number) => {
  try {
    await api.delete(`/produto/categorias/${categoryId}`);
